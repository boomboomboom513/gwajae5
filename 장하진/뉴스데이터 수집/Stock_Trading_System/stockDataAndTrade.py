...
def uploadStockData(cur, conn, loadStockData, stockDataTableName):
    sql = 'insert into '+str(stockDataTableName)\
        +' (code,name,date,time,startprice,highprice,lowprice,endprice,tradecount,tradeprice,gaein,foreigner,agency,finance,insurance,commitment,bank,etcfinance,pensionfund,etccorporation,etcforeigner,samofund)'\
    ...
    for i in range(0, loadStockData.shape[0]):
        cur.execute(sql, (loadStockData['종목코드'][i],loadStockData['종목명'][i],loadStockData['날짜'][i],loadStockData['시간'][i],loadStockData['시가'][i]
                        ...
                        ,loadStockData['개인'][i],loadStockData['외국인'][i],loadStockData['기관계'][i],loadStockData['금융투자'][i],loadStockData['보험'][i]
                        ,loadStockData['투신'][i],loadStockData['은행'][i],loadStockData['기타금융'][i],loadStockData['연기금'][i],loadStockData['기타법인'][i]
                        ,loadStockData['기타외인'][i],loadStockData['사모펀드'][i]))
    conn.commit()
...
def uploadMariaDB(stockNameList, stockDataTableNameList, newsDataTableNameList):
    ...
    for nm, tn in zip(stockNameList, stockDataTableNameList):
        loadStockData = pd.read_csv('./hapstockdata/'+str(nm)+' 완성 데이터.csv', encoding='euc-kr')
...
while True:
    ...
    if 1530 >= timeNow.hour * 100 + timeNow.minute >= 900:
        print('[시스템]:'+str(timeNow.hour)+'시 '+str(timeNow.minute + 1)+'분 실시간 데이터 요청하는 중')
