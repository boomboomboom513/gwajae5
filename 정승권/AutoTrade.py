...
myToken = "xoxb-5602380050838-5602602185974-G7gPuaealNVuIreolkvutHIz"
def dbgout(message):
...
def rq6033(self, retcode):
    ...
    for i in range(cnt):
        ...
        amount = self.objRq.GetDatavalue(7, i) # 체결 잔고 수량
...
def get_target_price(code):
    """매수 목표가를 반환한다."""  # 이 부분 강화학습 or lstm or cnn으로 변경
    try:
...
if __name__ == '__main__': 
    try:
        symbol_list = ['A005930', 'A108860'] # 삼성, 셀바스 AI
        ...
        while True:
            ...
            t_sell = t_now.replace(hour=14, minute=59, second=0, microsecond=0)
            t_exit = t_now.replace(hour=15, minute=00, second=0,microsecond=0)
            today = datetime.today().weekday()
            if today == 5 or today == 6:  # 토요일이나 일요일이면 자동 종료
                printlog('Today is', 'Saturday.' if today == 5 else 'Sunday.')
                sys.exit(0)
            if t_9 < t_now < t_start and soldout == False: # 추후에 주석처리 해야함
                soldout = True
                sell_all()                                  # 여기까지
            if t_start < t_now < t_sell :  # AM 09:00 ~ PM 03:20 : 매수
                ...
                if t_now.minute == 30 and 0 <= t_now.second <= 5: 
                    ...
                    time.sleep(5)
            if t_sell < t_now < t_exit:  # PM 03:15 ~ PM 03:20 : 일괄 매도  # 여기도 주석처리
                if sell_all() == True:
